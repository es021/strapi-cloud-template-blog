...
module.exports = {
  ...
  register({ strapi }) {
    ...
    strapi.log.info(
      `[GCS ENV CHECK] GCS_BUCKET_NAME:${bucketName || "(not set)"}`,
    );
    strapi.log.info(
      `[GCS ENV CHECK] CDN_BASE_URL:${cdnBaseUrl || "(not set)"}`,
    );
    strapi.log.info(
      `[GCS ENV CHECK] GCS_SERVICE_ACCOUNT set:${!!serviceAccountRaw}`,
    );
    strapi.log.info(
      `[GCS ENV CHECK] GCS_SERVICE_ACCOUNT valid JSON:${serviceAccountParsed}`,
    );
    if (serviceAccountError) {
      strapi.log.error(
        `[GCS ENV CHECK] GCS_SERVICE_ACCOUNT parse error:${serviceAccountError}`,
      );
    }
